...
def plot_times(num_tasks_list, times_service, times_filter):
    ...
    plt.title('Tiempos de procesamiento vs cantidad de datos (RabbitMQ)')
    plt.xlabel('Cantidad de datos')
    plt.ylabel('Tiempo (segundos)')
    plt.grid(True)
    plt.legend()
    plt.savefig('RabbitMQ/multiple_nodes_times.png')
    plt.close()
...
def plot_speedups(num_tasks_list, speedup_service, speedup_filter):
    ...
    plt.title('Speedup vs cantidad de datos (RabbitMQ)')
    plt.xlabel('Cantidad de datos')
    plt.ylabel('Speedup')
    plt.grid(True)
    plt.legend()
    plt.savefig('RabbitMQ/multiple_nodes_speedup.png')
    plt.close()

if __name__ == "__main__":
    filename = 'REDIS/test_results_m.txt'  # Cambia si usas otro archivo
